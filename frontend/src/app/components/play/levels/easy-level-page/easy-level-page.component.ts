...
@Component({
	...
	styleUrls: ['./easy-level-page.component.css', '../../../../../styles.css']
})
export class EasyLevelPageComponent implements OnInit {
	...
	generate() {
		...
		for (let i = 0; i < this.answers.length; i++) {
			let available=getRandomInt(4);

			// @ts-ignore
			document.getElementById(this.answers[i][0]).removeAttribute('disabled');
			// @ts-ignore
			document.getElementById('sentence'+this.answers[i][0]).removeAttribute('disabled');
			// @ts-ignore
			document.getElementById('structure'+this.answers[i][0]).removeAttribute('disabled');
			// @ts-ignore
			document.getElementById('word'+this.answers[i][0]).removeAttribute('disabled');
			...
			if (available==0) this.changeID.push(this.answers[i][0]);
			else if (available==1) this.changeID.push('structure' + this.answers[i][0]);
			else if (available==2) this.changeID.push('sentence' + this.answers[i][0]);
			else this.changeID.push('word' + this.answers[i][0]);

			this.changeNumber[0].push(i);
			this.changeNumber[1].push(available);
